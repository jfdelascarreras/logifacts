...
UPS_HOME = "https://www.ups.com/us/en/Home.page"
UPS_ORIGIN = "https://www.ups.com"
...
def any_visible(page: Page, selectors: Iterable[str], timeout: int = 1200) -> bool:
    for selector in selectors:
        try:
            locator = page.locator(selector).first
            locator.wait_for(state="visible", timeout=timeout)
...
def maybe_login(page: Page, username: Optional[str], password: Optional[str], wait_seconds: int) -> None:
    ...
    # Cookie banners can overlap/steal clicks on UPS home.
    click_first(
        page,
        [
    ...
    if not login_clicked:
        # Sometimes the visible header button opens a panel first; try panel login.
        click_first(
            page,
            [
        ...
        login_clicked = click_first(
            page,
            [
                ":has-text('Hello there') button:has-text('Log In')",
                ":has-text('Hello there') a:has-text('Log In')",
                "button:has-text('Log In')",
                "a:has-text('Log In')",
            ],
            "Log In (panel)",
        ...
        if not login_clicked:
            log("No login CTA found; assuming session is already authenticated")
            return

    if not username or not password:
        raise RuntimeError("UPS_USERNAME and UPS_PASSWORD are required when not already logged in")

    # UPS sometimes shows a second "Log In" CTA card before actual credential fields.
    click_first(
        page,
        [
            ":has-text('Hello there') button:has-text('Log In')",
    ...
    # If UPS keeps us on the marketing card, retry visible login CTAs on-page.
    if not any_visible(page, user_selectors, timeout=2500):
        log("Login form not visible yet, retrying on-page login CTAs")
        click_first(
            page,
            [
                "button[aria-label*='close' i]",
        ...
        click_first(
            ...
            "Log In (fallback retry)",
            timeout=4000,
        )

    log("Waiting for UPS username field...")
    try:
        user_field = first_visible(page, user_selectors, timeout=15000)
    except Exception:
        # Last-resort fallback: pick first visible non-password input.
        fallback_inputs = page.locator(
            "input:not([type='password']):not([type='hidden']):not([type='checkbox'])"
        )
        if fallback_inputs.count() == 0:
            raise
        user_field = fallback_inputs.first
...
def open_pay_bill(page: Page, context: BrowserContext, wait_seconds: int) -> Page:
    pay_bill_selectors = [
        ":has-text('Hello there') a:has-text('Pay Bill')",
        ":has-text('Hello there') button:has-text('Pay Bill')",
        "a[href*='bill' i]",
        "a[href*='billing' i]",
        "text=/Pay Bill/i",
        "role=link[name=/Pay Bill/i]",
        "role=button[name=/Pay Bill/i]",
    ]
    ...
    if not clicked:
        log("Could not click Pay Bill on first try; reopening account menu and retrying.")
        click_first(
            page,
            [
                "button[aria-label*='Account' i]",
        ...
        if not clicked:
            raise RuntimeError(
                "could not click Pay Bill from the account menu. "
                "UPS account panel likely changed; capture a screenshot/log and update selectors."
            )
...
def open_billing_center(page: Page, context: BrowserContext, wait_seconds: int) -> Page:
    try:
    ...
    if not clicked:
        log(
            "Could not click Billing Center automatically. "
            f"You can click it manually in the open browser (waiting up to {wait_seconds}s)."
        )
        page.wait_for_timeout(wait_seconds * 1000)
        clicked = click_first(
            page,
...
def main() -> int:
    args = parse_args()
    username = os.getenv("UPS_USERNAME") or os.getenv("CLUB_COLORS_USER")
    password = (
        os.getenv("UPS_PASSWORD")
        or os.getenv("password_Club_Colors")
        or os.getenv("password_Club_colors")
    )
    ...
    with sync_playwright() as p:
        ...
        # Prevent browser-level geolocation permission prompts from blocking UI actions.
        context.grant_permissions(["geolocation"], origin=UPS_ORIGIN)
        ...
        try:
            ...
            page = open_pay_bill(page, context, args.headed_wait_seconds)
            page = open_billing_center(page, context, args.headed_wait_seconds)
