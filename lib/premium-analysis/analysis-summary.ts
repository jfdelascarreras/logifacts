...
export type { AgentsAnalysisExtensions } from '@/lib/premium-analysis/agents-types'
...
export type ChargeTaxonomyValue = {
  transportation_mode: string
  category_1: string
  category_2: string
  category_3: string
...
export type InvoiceAnalysisSummary = {
  ...
  carrierMix?: AgentsAnalysisExtensions['carrierMix']
  anomalyFlags?: AgentsAnalysisExtensions['anomalyFlags']
...
export function lookupChargeTaxonomyForRecord(
  lookup: InvoiceTaxonomyLookup,
  rec: InvoiceRecord
): ChargeTaxonomyValue | undefined {
  return lookupChargeTaxonomy(lookup, rec['Carrier Name'] ?? '', rec['Charge Description'] ?? '')
}
